...
GMAIL_APP_PASSWORD = "" # App password set in Google account
...
@app.route('/admin')
def admin():
    """Render admin page for managing claimed items"""
...
@app.route('/api/items/<item_id>/deliver', methods=['POST'])
def deliver_item(item_id):
    """Mark an item as delivered.
    
    This will hide the item from the main home page.
    """
    try:
        items = read_items()
        item = next((i for i in items if i.get('id') == item_id), None)
        ...
        if not item.get('claim_code'):
            return jsonify({'error': 'Item has not been claimed yet.'}), 400

        if item.get('delivered'):
            return jsonify({'error': 'Item already marked as delivered.'}), 409

        item['delivered'] = True
        item['delivered_at'] = int(time.time())

        if not write_items(items):
            return jsonify({'error': 'Failed to mark as delivered. Try again.'}), 500

        return jsonify({'success': True, 'message': 'Item marked as delivered'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
